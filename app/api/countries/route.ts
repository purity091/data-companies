...
export async function GET() {
  if (isPreviewMode()) return NextResponse.json({ mode: "preview", data: previewStore.listCountries() });
  if (!isDatabaseConfigured()) return databaseNotConfiguredResponse();

  try {
    return NextResponse.json({ data: serializeCompany(await catalogService.list("countries")) });
  } catch (error) {
