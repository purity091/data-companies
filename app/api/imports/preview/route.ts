...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ data: result.company, format: result.format, issues: result.issues, canCommit: Boolean(result.company && !result.issues.some((issue) => issue.severity === "error")) });
  } catch (error) {
