...
export async function GET() {
  if (isPreviewMode()) return NextResponse.json({ mode: "preview", data: previewStore.listMarkets() });
  if (!isDatabaseConfigured()) return databaseNotConfiguredResponse();

  try {
    return NextResponse.json({ data: serializeCompany(await catalogService.list("markets")) });
  } catch (error) {
