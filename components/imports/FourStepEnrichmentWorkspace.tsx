...
function IdentityReview({ preview, enrichment, sources }: { preview: PreviewCompany; enrichment: LlmEnrichmentBundle; sources: PreviewSource[] }) { const identity = enrichment.identity; return <div className="space-y-5"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><ReviewField label="الاسم" value={preview.name} /><ReviewField label="الاسم القانوني" value={preview.legalName} /><ReviewField label="المعرّف الدائم" value={identity?.companyKey} /><ReviewField label="الموقع" value={preview.websiteUrl} /><ReviewField label="الدولة" value={preview.countryName} /><ReviewField label="الصناعة" value={preview.industryName} /><ReviewField label="المقر" value={identity?.headquarters} /><ReviewField label="نوع الشركة" value={identity?.companyType} /><ReviewField label="الموظفون" value={identity?.employeeCount} /><ReviewField label="سنة التأسيس" value={preview.foundedYear} /><ReviewField label="نموذج النشاط" value={identity?.businessModel} /><ReviewField label="المجال الاستراتيجي" value={identity?.strategicDomain} /><ReviewField label="نطاق الوصول" value={identity?.reachScope} /></div><TextBox label="الرؤية" value={identity?.vision} /><TextBox label="اللمحة التعريفية" value={preview.description} /><div className="grid gap-5 lg:grid-cols-2"><TagList title="الأسواق" items={identity?.markets ?? []} /><TagList title="الأسواق الحالية" items={identity?.currentMarkets ?? preview.markets} /></div><div className="grid gap-5 lg:grid-cols-2"><TagList title="تقنيات الشركة" items={identity?.techStack ?? []} /><TagList title="قنوات التسويق" items={identity?.marketingChannels ?? []} /></div><SourceList sources={sources} /> </div>; }
