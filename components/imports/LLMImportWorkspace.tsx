...
export function LLMImportWorkspace() {
  ...
  function handleFileUpload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (e) => {
      ...
      if (content) {
        setRawText(content);
        setStatus(`📁 تم تحميل الملف: ${file.name}`);
      }
    };
  ...
  async function preview() {
    ...
    try {
      ...
      setCompany(body.data as ImportedCompany | null);
      setEnrichment(body.enrichment as Record<string, unknown> | null);
      setIssues((body.issues || []) as Issue[]);
      setFormat(body.format || null);
      setStatus(body.canCommit ? "تم تحليل ملف الشركة بنجاح. راجع البيانات المستخرجة قبل الحفظ." : "يحتاج المحتوى إلى تصحيح قبل الحفظ.");
  ...
  async function commit() {
    ...
    try {
      const response = await fetch("/api/imports/commit", {
        ...
        body: JSON.stringify({ company, companyId: companyId || undefined, enrichment: enrichment || undefined }),
      });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "تعذر حفظ الشركة");
      setStatus(`✓ تم حفظ ${body.data.name} في قاعدة البيانات بنجاح.`); setCompany(null); setEnrichment(null); setRawText("");
    ...
    <main className="min-h-screen bg-slate-50 px-5 py-8 sm:px-8 lg:px-10">
      <div className="mx-auto max-w-7xl">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <Link href="/companies" className="text-sm font-bold text-sky-700">← العودة إلى الشركات</Link>
            <h1 className="mt-4 text-3xl font-black tracking-tight text-slate-950">رفع واستيراد شركة عبر JSON</h1>
            <p className="mt-2 max-w-3xl text-sm font-medium leading-7 text-slate-600">
              رفع ملف JSON مجهز أو لصق بيانات الشركة بالكامل. سيتم تحليل البيانات الأساسية والإثراء، وحفظها مباشرة في قاعدة البيانات.
            </p>
          </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          <section className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm sm:p-7">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-lg font-black text-slate-950">محتوى JSON / النص الخام</h2>
                <p className="mt-1 text-xs font-medium text-slate-500">اختر ملف .json من جهازك أو ألصق الكود مباشرة.</p>
              </div>
              <div className="flex gap-2">
                <label className="cursor-pointer rounded-lg bg-sky-700 px-3 py-2 text-xs font-bold text-white hover:bg-sky-800">
                  📁 رفع ملف JSON
                  <input type="file" accept=".json,application/json" onChange={handleFileUpload} className="hidden" />
                </label>
                <button type="button" onClick={() => setRawText(example)} className="rounded-lg bg-slate-100 px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-200">تحميل مثال</button>
              </div>
            </div>
            <textarea value={rawText} onChange={(event) => setRawText(event.target.value)} className="mt-5 min-h-[480px] w-full rounded-2xl border border-slate-200 bg-slate-50 p-4 font-mono text-sm leading-7 text-slate-800 outline-none focus:border-sky-400 focus:ring-4 focus:ring-sky-100" placeholder="الصق هنا كائن JSON الكامل الخاص بالشركة..." />
            <button type="button" onClick={() => void preview()} disabled={!rawText.trim() || loading} className="mt-4 w-full rounded-xl bg-slate-950 px-5 py-3 text-sm font-bold text-white hover:bg-sky-700 disabled:cursor-not-allowed disabled:opacity-50">{loading ? "جارٍ المعالجة..." : "تحليل ومعاينة ملف JSON"}</button>
          </section>

          <section className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-sm sm:p-7">
            <div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-black text-slate-950">المراجعة قبل الحفظ في قاعدة البيانات</h2><p className="mt-1 text-xs font-medium text-slate-500">سيتم حفظ جميع الحقول والإثراء المرفق.</p></div>{format && <span className="rounded-full bg-sky-50 px-3 py-1 text-xs font-bold text-sky-700">{format}</span>}</div>
            ...
            {!company ? <div className="mt-6 rounded-2xl bg-slate-50 px-5 py-16 text-center text-sm font-medium leading-7 text-slate-500">قم برفع ملف JSON أو لصق محتواه ثم اضغط "تحليل ومعاينة" لمعاينة البيانات قبل الحفظ.</div> : <div className="mt-6 space-y-5">
              <div className="rounded-2xl bg-slate-50 p-5">
                <h3 className="text-xl font-black text-slate-950">{company.name}</h3>
                <p className="mt-2 text-sm leading-7 text-slate-600">{company.description || "لا يوجد وصف"}</p>
                <div className="mt-4 flex flex-wrap gap-2">{[company.countryName, company.industryName, company.foundedYear?.toString()].filter(Boolean).map((item) => <span key={item} className="rounded-full bg-white px-3 py-1 text-xs font-bold text-slate-600">{item}</span>)}</div>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                <PreviewCount label="الأشخاص والقيادة" value={company.people.length} />
                <PreviewCount label="المستثمرون" value={company.investors.length} />
                <PreviewCount label="الأسواق" value={company.markets.length} />
                <PreviewCount label="المصادر" value={company.sources.length} />
              </div>
              {enrichment && (
                <div className="rounded-2xl border border-sky-200 bg-sky-50 p-4 text-xs leading-6 text-sky-950">
                  <b>✨ حزمة الإثراء (Enrichment Bundle) مكتشفة بالكامل!</b>
                  <p className="mt-1 text-sky-800">تتضمن بيانات الهوية، نموذج العمل، المالية، وتوصيف SWOT والمخاطر. سيتم حفظها في الجداول المخصصة.</p>
                </div>
              )}
              <PreviewList title="الأشخاص" values={company.people.map((item) => `${item.fullName}${item.jobTitle ? ` — ${item.jobTitle}` : ""}`)} />
              <PreviewList title="المستثمرون" values={company.investors.map((item) => item.name)} />
              <PreviewList title="المصادر" values={company.sources.map((item) => item.url)} links />
              ...
              <button type="button" onClick={() => void commit()} disabled={loading || issues.some((issue) => issue.severity === "error")} className="w-full rounded-xl bg-emerald-600 px-5 py-3 text-sm font-bold text-white hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50">اعتماد وحفظ الشركة في قاعدة البيانات</button>
            </div>}
          </section>
        </div>
      </div>
