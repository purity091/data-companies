...
function mapPeople(value: unknown) {
  return asList(value).map((item) => {
    ...
    return {
      ...
      jobTitle: text(pick(item, ["jobtitle", "title", "role", "position", "المسمىالوظيفي"])) || null,
...
function normalizeRecord(input: UnknownRecord, rawText: string): unknown {
  const nested = pick(input, ["company", "companyrecord", "data"]);
  const record = isRecord(nested) ? nested : input;
  const foundedRaw = pick(record, ["foundedyear", "founded", "yearfounded", "سنةالتأسيس"]);
  const foundedYear = Number(text(foundedRaw));
  return {
    name: text(pick(record, ["name", "companyname", "اسم الشركة", "اسمالشركة"])) || "",
    legalName: text(pick(record, ["legalname", "الاسم القانوني", "الاسمالقانوني"])),
    ...
    people: mapPeople(pick(record, ["people", "keypeople", "team", "الأشخاص"])),
    ...
    sources: mapSources(pick(record, ["sources", "references", "المصادر"]), rawText),
  };
}

function parseJson(rawText: string): UnknownRecord | null {
  const withoutFence = rawText.replace(/^\s*```(?:json)?\s*/i, "").replace(/\s*```\s*$/i, "").trim();
  ...
  for (const candidate of candidates) {
    try {
      ...
      // Try the next representation, then fall back to markdown/text parsing.
    }
  }
  return null;
}
...
function parseLabeledText(rawText: string): UnknownRecord {
  ...
  const aliases: Record<string, string> = {
    ...
    industry: "industry", sector: "industry", "الصناعة": "industry", "سنة التأسيس": "foundedYear", "founded year": "foundedYear",
  ...
  for (const line of rawText.split(/\r?\n/)) {
    ...
    if (key && ["people", "investors", "markets", "sources"].includes(key)) {
      const listKey = key as "people" | "investors" | "markets" | "sources";
      activeList = listKey;
      if (!output[key]) output[key] = [];
      if (separator >= 0 && clean.slice(separator + 1).trim()) (output[key] as unknown[]).push(clean.slice(separator + 1).trim());
      continue;
    }
    if (activeList && (line.trim().startsWith("-") || line.trim().startsWith("*") || line.trim().startsWith("•"))) {
...
export function parseLlmImport(rawText: string): ParsedLlmImport {
  ...
  if (!result.data.industryName) issues.push({ severity: "warning", field: "industryName", message: "الصناعة غير محددة." });
